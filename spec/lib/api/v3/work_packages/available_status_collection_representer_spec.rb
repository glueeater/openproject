...
describe ::API::V3::WorkPackages::AvailableStatusCollectionRepresenter do
  ...
  context 'generation' do
    ...
    it 'should have link to self' do
      expect(parse_json(subject)['_links']['self']['href']).to match(%r{api/v3/work_packages/1/available_statuses$})
    end

    it 'should have link to work_package' do
      expect(parse_json(subject)['_links']['work_package']['href']).to match(%r{api/v3/work_packages/1$})
    end
  end
end
